...
if [ $exit_code -eq 0 ]; then
	...
	if [ $exit_code -eq 0 ]; then
		...
		qemu-system-x86_64 -D ./qemu-test.log -d cpu,exec,int -drive file=test/test_drive.img,format=raw -drive file=linl.iso,format=raw,media=cdrom -no-reboot -no-shutdown -m 6G -cpu host -monitor stdio -enable-kvm
